...
var _ = Describe("Feast Modular Upgrade scenario Testing", Ordered, func() {
	...
	runFeastModuleOperatorTest := func() {
		...
		CheckDeployment(applicationsNamespace, feastOperatorDeploymentName)

		By("Verifying feast-operator-controller-manager deployment and pods have app.kubernetes.io/name=feast-operator,control-plane=controller-manager selector label")
		ValidateDeploymentAndPodsSelectorLabels(
